...
mysql-dump:
	...
	@docker exec $(shell docker-compose ps -q mysqldb) mysqldump test -u"$(MYSQL_ROOT_USER)" -p"$(MYSQL_ROOT_PASSWORD)" > $(MYSQL_DUMPS_DIR)/db.sql
	@make resetOwner
...
test:
	...
	@make resetOwner
	web/app/vendor/bin/codecept run characterisation -c web/app/codeception.yml
resetOwner:
